...
const mpesaStatusStore = {};
const checkAdminBalance = async (token, amount) => {
    ...
    if (typeof decimals !== 'number')
        throw new Error('Decimals undefined');
    ...
    if (token === 'PAS') {
        ...
        const accountInfo = await assetHubApi.query.system.account(adminAddress);
        ...
        const accountAsset = await assetHubApi.query.assets.account(assetId, adminAddress);
...
const checkUserBalance = async (userAddress, token, amount) => {
    const decimals = ASSET_DECIMALS[token];
    if (typeof decimals !== 'number')
        throw new Error('Decimals undefined');
    const requiredAmount = BigInt(Math.floor(amount * Math.pow(10, decimals)));
    ...
    if (token === 'PAS') {
        ...
        const accountInfo = await assetHubApi.query.system.account(userAddress);
        const balance = accountInfo.data.free.toBigInt();
        const frozen = (accountInfo.data.frozen?.toBigInt?.() ?? accountInfo.data.miscFrozen?.toBigInt?.() ?? BigInt(0));
        const availableBalance = balance - frozen;
        const balanceAfterTx = availableBalance - requiredAmount;
        ...
        const accountAsset = await assetHubApi.query.assets.account(assetId, userAddress);
        if (accountAsset.isNone) {
            return {
                hasEnough: false,
                currentBalance: '0',
 ...
 */
export const buyCryptoController = async (req, res) => {
    try {
...
const processPayment = async (merchantId) => {
    ...
    try {
        ...
        if (data.token === 'PAS' &&
            ...
            if (!assetId)
                throw new Error(`Asset ID not found for ${data.token}`);
 ...
 */
export const sellCryptoController = async (req, res) => {
    try {
        ...
        const merchantRequestId = b2cResponse.data.OriginatorConversationID ||
 ...
 */
export const callbackUrlController = (req, res) => {
 ...
 */
export const b2cResultController = (req, res) => {
 ...
 */
export const b2cTimeoutController = (req, res) => {
 ...
 */
export const mpesaStatusController = (req, res) => {
    ...
    if (Array.isArray(merchantRequestId))
        merchantRequestId = merchantRequestId[0];
    ...
    try {
        ...
        if (token === 'PAS') {
            ...
            if (assetHubApi &&
                ...
                const accountAsset = await assetHubApi.query.assets.account(assetId, address);
                if (accountAsset.isNone) {
                    return res.json({
                        status: 'success',
                        address,
 ...
 * GET /api/v1/rates
 */
export const getRatesController = (req, res) => {
 ...
 * GET /api/v1/tokens
 */
export const getTokensController = (req, res) => {
    console.log('[getTokensController] Fetching supported tokens');
    const tokens = Object.keys(EXCHANGE_RATES).map(token => {
        const decimals = ASSET_DECIMALS[token];
        if (typeof decimals !== 'number')
            throw new Error('Decimals undefined');
 ...
 */
export const getAdminAddressController = (req, res) => {
    try {
        const keyring = new Keyring({ type: 'sr25519' });
        const safeMnemonic = sanitizeMnemonic(MNEMONIC);
        if (!mnemonicValidate(safeMnemonic)) {
 ...
 */
export const getPendingTransactionsController = (req, res) => {
 ...
 */
export const getTransactionHistoryController = (req, res) => {
 ...
 */
export const getQuoteController = (req, res) => {
    try {
 ...
 */
export const updateRatesController = (req, res) => {
    try {
 ...
 */
export const getTransactionController = (req, res) => {
    try {
 ...
 */
export const cancelTransactionController = (req, res) => {
    try {
        const { merchantRequestId } = req.params;
        if (!merchantRequestId) {
            return res.status(400).json({
